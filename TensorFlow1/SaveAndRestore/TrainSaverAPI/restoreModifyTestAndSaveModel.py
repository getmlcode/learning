...
parser.add_argument('--model', 
                    ...
                    default='simpleModel', 
                    required=False, 
                    help='Model Name (file name with .meta extension)'
                    )
...
with tf.Session() as sess:
    ...
    nonZeroBias = tf.Variable(tf.ones((10,)),name="nonZeroBias") #Add new variable
    ...
    result = sess.run([Output,x,w],{x:dataPoint})
    
    addBias = tf.add(Output,nonZeroBias,name='addNzBias')

    ModifiedOutput = tf.nn.softmax(addBias,name="Softmax") #Add an operation to graph
    NewResult = sess.run(ModifiedOutput,{x:dataPoint})
    tf.add_to_collection('v',ModifiedOutput)
